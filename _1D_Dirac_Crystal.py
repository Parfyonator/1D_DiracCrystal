...
class Crystal:
    ...
    def WhittakerW(k: float, m: float, x: float) -> float:
        ...
        """
        return np.exp(-x/2) * pow(x, m + 0.5) * special.hyperu(0.5 + m - k, 1 + 2*m, x)
    ...
    def V(self, x):
        ...
        return self.Z_p * self.FINE_STRUCTURE_CONSTANT * \
               sum([z / (np.abs(x-a) + self.DELTA) for a, z in zip(self.A, self.Z)])
    ...
    def plot_LDOS(self, level_rng, filenames=None):
        ...
        if not filenames:
            ...
            for pair in re.findall('\((.*?)\)', filenames[0], re.DOTALL):
                ...
                A.append(float(a))
                Z.append(float(z))
            delta = float(re.findall('delta=(.*?)/', self.crystal_dir(), re.DOTALL)[0])
